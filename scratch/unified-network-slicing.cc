...
private:
  ...
  bool m_enableQosSteer = false; //this will be enabled only if steer mode is active!!! Did not implement Qos Split yet...
...
GmaSimWorker::UpdateStatus(){
  ...
  if (ifTraceFile.good ())
  {
    ...
    seconds = time (NULL);
    myfile << (int)seconds << "/"<< m_stopTime.GetSeconds() << "/" <<m_stopTime.GetSeconds() <<"\n";
    myfile.close();
  ...
  else
  {
    ...
    myfile << (int)seconds << "/"<< Simulator::Now ().GetSeconds () << "/" <<m_stopTime.GetSeconds() <<"\n";
    myfile.close();
  }
...
GmaSimWorker::LogLocations ()
{
    ...
    if(Simulator::Now ().GetSeconds () < 0.1)
    {
         ...
         for (uint32_t apInd = 0; apInd < m_clientNodes.GetN(); apInd++)
        {
            ...
            // std::cout << "UE "<< apInd+1 << " location: [" << +uepos.x << "," << +uepos.y << "," << +uepos.z << "\n";
...
GmaSimWorker::ParseJsonConfig ()
{
  ...
  m_sliceNum = jsonConfig["num_slices"].get<int>();
  std::cout << "num_slices: " << m_sliceNum << std::endl;
  ...
  if (m_sliceNum > 0)
  {
    for(uint64_t ind = 0; ind < m_sliceNum; ind++)
    {
      ...
      if(perSliceConfig->m_transport_protocol.compare(0, 3, "tcp") == 0 || perSliceConfig->m_transport_protocol.compare(0, 3, "TCP") == 0 || perSliceConfig->m_transport_protocol.compare(0, 3, "Tcp") == 0)
      {
        ...
        TypeId tcpTid;
        NS_ABORT_MSG_UNLESS(TypeId::LookupByNameFailSafe(perSliceConfig->m_transport_protocol, &tcpTid),
                            "TypeId " << perSliceConfig->m_transport_protocol << " not found");
        Config::SetDefault("ns3::TcpL4Protocol::SocketType",
                          TypeIdValue(TypeId::LookupByName(perSliceConfig->m_transport_protocol)));
      }
      ...
      std::cout <<"slice: "<< +ind << " num_users = " << sliceInfo->m_numUsers << "\n"
                <<"slice: "<< +ind << " dedicated_rbg = " << sliceInfo->m_dedicatedRbg << "\n"
                <<"slice: "<< +ind << " shared_rbg = " <<  sliceInfo->m_prioritizedRbg << "\n"
                <<"slice: "<< +ind << " packet_size_bytes = " << perSliceConfig->m_packetSize << "\n"
                <<"slice: "<< +ind << " transport_protocol = " << perSliceConfig->m_transport_protocol << "\n"
                <<"slice: "<< +ind << " udp_poisson_arrival = " << perSliceConfig->m_poissonArrival << "\n"
                <<"slice: "<< +ind << " min_udp_rate_per_user_mbps = " << perSliceConfig->m_minUdpRateMbps << "\n"
                <<"slice: "<< +ind << " max_udp_rate_per_user_mbps = " << perSliceConfig->m_maxUdpRateMbps << "\n"
                <<"slice: "<< +ind << " qos_loss_target = " << perSliceConfig->m_qosLossTarget <<  "\n"
                <<"slice: "<< +ind << " qos_delay_violation_target = " << perSliceConfig->m_qosDelayViolationTarget <<  "\n"
                <<"slice: "<< +ind << " qos_delay_requirement_ms = " << perSliceConfig->m_qosDelayRequirementMs <<  "\n"
                <<"slice: "<< +ind << " delay_test_1_thresh_ms = " << perSliceConfig->m_delaythresh1 <<  "\n"
                <<"slice: "<< +ind << " delay_test_2_thresh_ms = " << perSliceConfig->m_delaythresh2 <<  "\n"
                <<"slice: "<< +ind << " downlink_mode = " << perSliceConfig->m_dlGmaMode <<  "\n"
                <<"slice: "<< +ind << " uplink_mode = " << perSliceConfig->m_ulGmaMode <<  "\n";
  ...
  {
    NS_FATAL_ERROR("num_slices must be greater than zero!");
  }
  ...
  std::cout << "Config loaded from 'env-configure.json':"<< "\n"
            << "simulation_time_s = " << m_stopTime.GetSeconds() <<  "\n"
            << "random_seed = " << random_seed <<  "\n"
            << "downlink = " << m_downlink <<  "\n"
            << "EnableRtsCts = " << m_enableRtsCts <<  "\n"
            << "max_wait_time_for_action_ms = " << m_action_wait_ms <<  "\n"
            << "num_users = " << m_numOfUsers <<  "\n";

  std::cout << "user_random_walk_speed = [" << m_userSpeedMin << "|" << m_userSpeedMax<< "]\n"
            << "user_random_walk_direction = [" << m_userDirectionMin << "|" << m_userDirectionMax<< "]\n"
            << "user_random_walk_distance = " << m_userWalkDistance << "\n"
            << "measurement_start_time_ms = " << m_measurement_start_time_ms <<  "\n"
            << "[GMA] enable_dynamic_flow_prioritization = " << m_dfp << "\n"
            << "[GMA] mx_algorithm = " << m_splittingAlgorithm << ""

            << "[GMA] measurement_interval_ms = " << gma_interval.GetMilliSeconds() <<  "\n"
            << "[GMA] measurement_guard_interval_ms = " << gma_guard.GetMilliSeconds() <<  "\n"

            << "[Wi-Fi] measurement_interval_ms = " << m_wifiMeasurementInterval.GetMilliSeconds() <<  "\n"
            << "[Wi-Fi] measurement_guard_interval_ms = " <<  wifi_guard.GetMilliSeconds() << "\n"
            
            << "[LTE] resource_block_num = " << m_lteRb <<  "\n"
            << "[LTE] measurement_interval_ms = " << m_lteUeMeasurementInterval.GetMilliSeconds() <<  "\n"
            << "[LTE] measurement_guard_interval_ms = " <<  lte_guard.GetMicroSeconds() << "\n";
  ...
  if(m_enableQosSteer)
  {
    int test_duration_ms =  jsonConfig["qos_requirement"]["test_duration_ms"].get<int>();
    if(test_duration_ms < 100 || test_duration_ms > 25000)
    {
      NS_FATAL_ERROR("test_duration_ms only support range from [100, 25000].");
    }


    double delay_violation_target = jsonConfig["qos_requirement"]["delay_violation_target"].get<double>();
    double loss_target = jsonConfig["qos_requirement"]["loss_target"].get<double>();

    std::cout << "[qos_requirement] test_duration_ms = " << test_duration_ms <<  "\n"
              << "[qos_requirement] delay_violation_target = " << delay_violation_target <<  "\n"
              << "[qos_requirement] loss_target = " << loss_target <<  "\n";
    Config::SetDefault ("ns3::LinkState::QosTestDurationUnit100ms", UintegerValue (test_duration_ms/100));
    Config::SetDefault ("ns3::GmaRxControl::QosDelayViolationTarget", DoubleValue (delay_violation_target));
    Config::SetDefault ("ns3::GmaRxControl::QosLossTarget", DoubleValue (loss_target));
...
GmaSimWorker::SaveConfigFile()
{
  ...
  if(m_radioType == WIFI_CID)
  {
    ...
    Config::SetDefault ("ns3::LinkState::DefaultLinkCid", UintegerValue (WIFI_CID));//default over wifi
  ...
  else if(m_radioType == CELLULAR_LTE_CID)
  {
    Config::SetDefault ("ns3::GmaTxControl::SplittingAlgorithm", EnumValue (GmaTxControl::DefaultLink));//Tx use default link
    Config::SetDefault ("ns3::GmaRxControl::SplittingAlgorithm", EnumValue (GmaRxControl::DefaultLink));//Rx use default link, no tsu update
    Config::SetDefault ("ns3::LinkState::DefaultLinkCid", UintegerValue (CELLULAR_LTE_CID));//default over LTE
    Config::SetDefault ("ns3::LinkState::FixDefaultLink", BooleanValue (true));//never update default link

    myfile << "Radio: lte Only\n";
    for (uint32_t i = 0; i < m_perSliceConfigList.size(); i++)
    {
  ...
  else if(m_radioType == CELLULAR_NR_CID)
  {
    Config::SetDefault ("ns3::GmaTxControl::SplittingAlgorithm", EnumValue (GmaTxControl::DefaultLink));//Tx use default link
    Config::SetDefault ("ns3::GmaRxControl::SplittingAlgorithm", EnumValue (GmaRxControl::DefaultLink));//Rx use default link, no tsu update
    Config::SetDefault ("ns3::LinkState::DefaultLinkCid", UintegerValue (CELLULAR_NR_CID));//default over NR
    Config::SetDefault ("ns3::LinkState::FixDefaultLink", BooleanValue (true));//never update default link

    myfile << "Radio: nr Only\n";
    for (uint32_t i = 0; i < m_perSliceConfigList.size(); i++)
    {
  ...
  {
    myfile << "Radio: all network\n";
    for (uint32_t i = 0; i < m_perSliceConfigList.size(); i++)
    {
      if(m_perSliceConfigList.at(i)->m_dlGmaMode == "auto")
      {
        //change dl mode for auto mode
        if(m_perSliceConfigList.at(i)->m_tcpData && m_downlink && !m_enableQosSteer)
        {
        ...
        myfile <<" Slice: " << +i << " Downlink GMA mode: auto(config as " << m_perSliceConfigList.at(i)->m_dlGmaMode << ")\n";

      }
      else
      {
        myfile <<" Slice: " << +i << " Downlink GMA mode:" << m_perSliceConfigList.at(i)->m_dlGmaMode << "\n";
      }
      ...
      if(m_perSliceConfigList.at(i)->m_ulGmaMode == "auto")
      {
        ...
        myfile <<" Slice: " << +i << " Uplink GMA mode: auto(config as " << m_perSliceConfigList.at(i)->m_ulGmaMode << ")\n";
      }
      else
      {
        myfile <<" Slice: " << +i << " Uplink GMA mode:" << m_perSliceConfigList.at(i)->m_ulGmaMode << "\n";
      }
    }
  ...
  for (uint32_t i = 0; i < m_perSliceConfigList.size(); i++)
  {
    ...
    {
      myfile <<" Slice: " << +i << " Traffic: high throughput (TCP) - " << m_perSliceConfigList.at(i)->m_transport_protocol << "\n";

    }
    else
    {
      myfile <<" Slice: " << +i << " Traffic: low-latency (UDP)" << "\n";
      myfile <<" Slice: " << +i << " Min Rate per user: " << m_perSliceConfigList.at(i)->m_minUdpRateMbps << " mbps\n";
      myfile <<" Slice: " << +i << " Max Rate per user: " << m_perSliceConfigList.at(i)->m_maxUdpRateMbps << " mbps\n";
    }
    myfile <<" Slice: " << +i << " Packet size: " << m_perSliceConfigList.at(i)->m_packetSize << " bytes\n";
  }

  if(m_enableQosSteer)
  {
    //TODO: make the default link configurable for QoS Steer
    Config::SetDefault ("ns3::LinkState::DefaultLinkCid", UintegerValue (CELLULAR_LTE_CID));//default link over lte
    Config::SetDefault ("ns3::LinkState::FixDefaultLink", BooleanValue (true));//never update default link
    Config::SetDefault ("ns3::LinkState::PreferLargeCidValue", BooleanValue(true));//larger cid -> high priority
    Config::SetDefault ("ns3::GmaRxControl::SplittingAlgorithm", EnumValue (GmaRxControl::QosSteer));//Qos algorithm
    myfile << "QoS Steer Mode!\n";
  }
  else
  {
    if(m_splittingAlgorithm.compare("gma") == 0)
    {
      Config::SetDefault ("ns3::GmaRxControl::SplittingAlgorithm", EnumValue (GmaRxControl::gma));
      Config::SetDefault ("ns3::MeasurementManager::MaxMeasureInterval", UintegerValue (10)); //gma1 requires measurement converge or max of 10 intervals.
      Config::SetDefault ("ns3::MeasurementManager::EnableSenderSideOwdAdjustment", BooleanValue (false)); //gma1 equalize owd at the receiver side.
      m_splittingBurst = 32;
    }
    else if (m_splittingAlgorithm.compare("gma2") == 0)
    {
      Config::SetDefault ("ns3::GmaRxControl::SplittingAlgorithm", EnumValue (GmaRxControl::gma2));
      Config::SetDefault ("ns3::MeasurementManager::MaxMeasureInterval", UintegerValue (1)); //gma2 does not the measurement interval to converge.
      Config::SetDefault ("ns3::MeasurementManager::EnableSenderSideOwdAdjustment", BooleanValue (true)); //gma2 equalize owd at the transmitter side.
    ...
    Config::SetDefault ("ns3::LinkState::DefaultLinkCid", UintegerValue (CELLULAR_LTE_CID));//default link over lte
    //Config::SetDefault ("ns3::LinkState::DefaultLinkCid", UintegerValue (WIFI_CID));//default link over wifi
  }
  ...
  myfile << "Wi-Fi down time after AP switch "<< m_wifiDownTime <<" ms!\n";
  ...
  myfile << "Wi-Fi Power AP switch:" << m_wifiApSwitchPowerThresh <<"dBm; Low Thresh:" << m_wifiLowPowerThresh << "dBm; High thresh:" << m_wifiHighPowerThresh <<"dBm\n";
  myfile << "Num of users: " << m_numOfUsers << "\n";
  myfile << "User speed min: " << m_userSpeedMin << " m/s\n";
  myfile << "User speed max: " << m_userSpeedMax << " m/s\n";
  myfile << "User direction min: " << m_userDirectionMin << " m/s\n";
  myfile << "User direction max: " << m_userDirectionMax << " m/s\n";
  myfile << "User walk distance: " << m_userWalkDistance << "m\n";
  myfile << "Simulation time: " << m_stopTime.GetSeconds() << " seconds\n";
  myfile << "D (Wi-Fi AP Grid size): " << m_distance << " meters\n";
  //myfile << "Acceptable edge owd: " << m_qosDelayRequirementMs << " ms\n";
  myfile << "Wi-Fi backhaul delay: " << m_wifiBackhaulDelay << "\n";
  myfile << "GMA split algorithm delay thresh: " << m_gmaDelaySplitThresh << " ms\n";

  myfile << "Wi-Fi Queue MaxDelay:" << wifiDelay << " ms" << ", Queue MaxSize:" << wifiQueueSize << " packets\n";
  ...
  if(m_networkSlicing)
  {
    ...
    for (int u = 0; u < m_numOfUsers; u++)
    {
      ...
      for (int b=0; b<numOfBears; b++)
      {
        ...
        if(b == 0)
        {
          pktFilter.typeOfService = 0x70; //AC_BE
          pktFilter.typeOfServiceMask = 0xff;
          tft->Add(pktFilter);
...
GmaSimWorker::InstallGmaInterface()
{
  ...
  for (int clientInd = 0; clientInd < m_numOfUsers; clientInd++)
  {
    ...
    {
      routerInterface->ConfigureRxAlgorithm(m_splittingBurst);//this will send TSU to allow the other side to split traffic
    }
    //the other mode, no thing to config...
    routerInterface->SetQosRequirement(m_perSliceConfigList.at(sliceId)->m_qosDelayRequirementMs, 
    ...
    {
      clientInterface->ConfigureRxAlgorithm(m_splittingBurst);//this will send TSU to allow the other side to split traffic, enable owd offset (ave owd - min owd)
    }
    //the other mode, no thing to config...

    clientInterface->SetQosRequirement(m_perSliceConfigList.at(sliceId)->m_qosDelayRequirementMs,
          m_perSliceConfigList.at(sliceId)->m_delaythresh1, 
          m_perSliceConfigList.at(sliceId)->m_delaythresh2, 
          m_perSliceConfigList.at(sliceId)->m_qosDelayViolationTarget,
          m_perSliceConfigList.at(sliceId)->m_qosLossTarget);
...
GmaSimWorker::InstallApplications()
{
  ...
  for (int clientInd = 0; clientInd < m_numOfUsers; clientInd++)
  {
    ...
    else
    {
      ...
      PoissonUdpClientHelper onoff (receiverAddress, port);
...
void
GmaSimWorker::ConfigureQosSteer(bool flag)
{
  m_enableQosSteer = flag;
}
...
GmaSimWorker::Run()
{
  ...
  Simulator::Run ();
  Simulator::Destroy ();
  std::cout << "Simulation end at " << m_stopTime.GetSeconds() << "s\n";
}

void
GmaSimWorker::TrafficSplittingDeployment ()
{
  ConfigureQosSteer(false);
...
{
  ConfigureQosSteer(true); //this will be enabled only if steer mode is active!!! Did not implement Qos Split yet...
...
void
GmaSimWorker::NetworkSlicingDeployment()
{
  ConfigureQosSteer(false);
...
main (int argc, char *argv[])
{
  ...
  else if(env_name.compare("qos_steer") == 0)
  {
    worker->QoSTrafficSteeringDeployment();
  }
  else if(env_name.compare("network_slicing") == 0)
  {
    worker->NetworkSlicingDeployment();
  }
  else
  {

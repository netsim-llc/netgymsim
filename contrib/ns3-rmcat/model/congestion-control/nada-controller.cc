...
NadaController::NadaController() :
    ...
    m_currInt{0},
    m_lossesSeen{false} {
        m_networkStatsCallback = ns3::MakeNullCallback<void, ns3::Ptr<ns3::NetworkStats>> ();
    }
...
void NadaController::updateBw(uint64_t deltaUs) {
    ...
    float tempBw =  m_currBw; //save the current bw for ml action.
    ...
    if(m_actionReceived)
    {
        //networkgym enabled, we will update bandwith from RL agent.
        m_currBwNada = m_currBw; //this is saved for measurement.
        m_currBw = tempBw;
    }
...
void NadaController::logStats(uint64_t nowUs, uint64_t deltaUs) const {
    ...
    if (!m_networkStatsCallback.IsNull())
    {   

        ns3::Ptr<ns3::NetworkStats> element = ns3::CreateObject<ns3::NetworkStats>("rmcat",  m_networkgymId, nowUs / 1000);
        element->Append("loglen", m_packetHistory.size());
        element->Append("qdel",  m_QdelayUs / 1000);
        element->Append("rtt",  m_RttUs / 1000);
        element->Append("ploss",  m_ploss);
        element->Append("plr",  m_plr);
        element->Append("xcurr",  m_Xcurr);
        element->Append("rrate",  m_RecvR);
        element->Append("srate",  m_currBw);
        element->Append("nada_srate",  m_currBwNada);

        element->Append("avgint",  m_avgInt);
        element->Append("curint",  m_currInt);
        element->Append("delta",  deltaUs / 1000);

        m_networkStatsCallback(element);

        /*ns3::Ptr<ns3::NetworkStats> elementTest = ns3::CreateObject<ns3::NetworkStats>("rmcat",  12, nowUs / 1000);
        elementTest->Append("loglen", m_packetHistory.size());
        elementTest->Append("qdel",  m_QdelayUs / 1000);
        elementTest->Append("rtt",  m_RttUs / 1000);
        elementTest->Append("ploss",  m_ploss);
        elementTest->Append("plr",  m_plr);
        elementTest->Append("xcurr",  m_Xcurr);
        elementTest->Append("rrate",  m_RecvR);
        elementTest->Append("srate",  m_currBw);
        elementTest->Append("avgint",  m_avgInt);
        elementTest->Append("curint",  m_currInt);
        elementTest->Append("delta",  deltaUs / 1000);

        m_networkStatsCallback(elementTest);*/
...
void
NadaController::ReceiveNetworkGymAction(const json& action)
{
    std::cout << this << " NADA RX Action " << action << std::endl;
    if (action == nullptr)
	{
		return;
	}
    m_actionReceived = true;
    m_currBw = action.get<double>();
    if (m_currBw > m_maxBw || m_currBw < m_minBw)
    {
        NS_FATAL_ERROR("The sending rate for rmcat must be between [" << m_minBw << ", " << m_maxBw << "].");
    }
}

void
NadaController::SetNetworkStatsCallback(uint64_t id, NetworkStatsCallback cb)
{
    m_networkStatsCallback = cb;
    m_networkgymId = id;
}

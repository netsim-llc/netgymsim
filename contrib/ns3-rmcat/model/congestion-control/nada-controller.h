...
public:
    ...
    typedef ns3::Callback<void, ns3::Ptr<ns3::NetworkStats> > NetworkStatsCallback;
    void SetNetworkStatsCallback(uint64_t id, NetworkStatsCallback cb);
    void ReceiveNetworkGymAction(const json& action);
...
private:
    ...
    float m_currBwNada; /**< calculated reference rate (r_ref in rmcat-nada) if ml action is active */
    ...
    NetworkStatsCallback m_networkStatsCallback; //callback that send measurement to networkgym
    uint64_t m_networkgymId;
    bool m_actionReceived = false;
};

...
public:
  ...
  void UpdateOwdFromProbe(uint32_t owdMs, uint8_t cid);//do nothing, qos measurement does not use ctrl msg
  void UpdateOwdFromAck(uint32_t owdMs, uint8_t cid);//used to compute the "synced" owd
  ...
  void QuitQosMeasurement (uint8_t cid);
private:

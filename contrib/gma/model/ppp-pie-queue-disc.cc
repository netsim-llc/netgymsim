...
#include "ns3/abort.h"
#include "ppp-pie-queue-disc.h"
#include "ns3/drop-tail-queue.h"

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("PppPieQueueDisc");

NS_OBJECT_ENSURE_REGISTERED (PppPieQueueDisc);

TypeId PppPieQueueDisc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PppPieQueueDisc")
    .SetParent<QueueDisc> ()
    .SetGroupName ("TrafficControl")
    .AddConstructor<PppPieQueueDisc> ()
    .AddAttribute ("MeanPktSize",
                   "Average of packet size",
                   UintegerValue (1000),
                   MakeUintegerAccessor (&PppPieQueueDisc::m_meanPktSize),
                   MakeUintegerChecker<uint32_t> ()) 
    .AddAttribute ("A",
                   "Value of alpha",
                   DoubleValue (0.125),
                   MakeDoubleAccessor (&PppPieQueueDisc::m_a),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("B",
                   "Value of beta",
                   DoubleValue (1.25),
                   MakeDoubleAccessor (&PppPieQueueDisc::m_b),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("Tupdate",
                   "Time period to calculate drop probability",
                   TimeValue (MilliSeconds (15)),
                   MakeTimeAccessor (&PppPieQueueDisc::m_tUpdate),
                   MakeTimeChecker ())
    .AddAttribute ("Supdate",
                   "Start time of the update timer",
                   TimeValue (Seconds (0)),
                   MakeTimeAccessor (&PppPieQueueDisc::m_sUpdate),
                   MakeTimeChecker ())
...
PppPieQueueDisc::PppPieQueueDisc ()
  : QueueDisc (QueueDiscSizePolicy::SINGLE_INTERNAL_QUEUE)
{
  NS_LOG_FUNCTION (this);
  m_uv = CreateObject<UniformRandomVariable> ();
  m_rtrsEvent = Simulator::Schedule (m_sUpdate, &PppPieQueueDisc::CalculateP, this);
...
void
PppPieQueueDisc::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  m_uv = 0;
  m_rtrsEvent.Cancel ();
...
Time
PppPieQueueDisc::GetQueueDelay (void)
{
  return m_qDelay;
}

int64_t
PppPieQueueDisc::AssignStreams (int64_t stream)
{
...
bool
PppPieQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
  ...
  else if ((m_activeThreshold == Time::Max () || m_active) && !isEct1 && DropEarlyPpp (item, nQueued.GetValue ()))
    {
...
void
PppPieQueueDisc::InitializeParams (void)
{
...
bool PppPieQueueDisc::DropEarly (Ptr<QueueDiscItem> item, uint32_t qSize)
{
...
bool PppPieQueueDisc::DropEarlyPpp (Ptr<QueueDiscItem> item, uint32_t qSize)
{
  NS_LOG_FUNCTION (this << item << qSize);
  if (m_burstAllowance.GetSeconds () > 0)
    {
  ...
  if(item->GetPacket()->PeekPacketTag(tag))//ppp tag exists, add to the m_bytesPerPpp
  {
    ...
    {
      return false; //don't drop the highest priority packet
    }

    //item->GetTimeStamp() returns the timestamp of the item(packet) at enqueue process.
    //qSize is the sum of bytes in the per priority queue: qsize=sum(m_bytesPerPpp)
    //CalculateDropDeficit(input1 = queueingDelay, input2 = queueSizeInBytes)
    //uint32_t dropDeficit = CalculateDropDeficit(CurrentTimeStamp() - item->GetTimeStamp(), qSize);
    uint32_t dropDeficit = round(p*qSize);
    if(dropDeficit > qSize)
    {
      return true; //drop packet due to delay violation
    }
    else if(dropDeficit > 0)
    {
      uint32_t pppIter = tag.GetPriority()+1; //start from the current packet priority + 1
      uint32_t lowPriorityBytes = 0; //# of bytes (in the queue) that has lower priority than the dequeued packet
      ...
      if(dropDeficit > lowPriorityBytes + item->GetPacket()->GetSize())//not enough low priority packets in the queue, drop this packet
      {
        return true; //drop packet
      }
      else
      {
        return false; //enough low priority packets in the queue, which will be dropped in the future, transmit this packet
      }
    }
    return false; //dropDeficit == 0, no drop.
  }
  return false;
}

void PppPieQueueDisc::CalculateP ()
{
  NS_LOG_FUNCTION (this);
  Time qDelay;
  double p = 0.0;
  ...
  m_qDelayOld = qDelay;
  m_rtrsEvent = Simulator::Schedule (m_tUpdate, &PppPieQueueDisc::CalculateP, this);
}

Ptr<QueueDiscItem>
PppPieQueueDisc::DoDequeue ()
{
  NS_LOG_FUNCTION (this);
  ...
  PppTag tag;
  if(item->GetPacket()->PeekPacketTag(tag))//ppp tag exists, reduce the m_bytesPerPpp
  {
    m_bytesPerPpp.at(tag.GetPriority()) -= item->GetPacket()->GetSize();
  }
...
bool
PppPieQueueDisc::CheckConfig (void)
{
  NS_LOG_FUNCTION (this);
  if (GetNQueueDiscClasses () > 0)
    {
      NS_LOG_ERROR ("PppPieQueueDisc cannot have classes");
      return false;
    }

  if (GetNPacketFilters () > 0)
    {
      NS_LOG_ERROR ("PppPieQueueDisc cannot have packet filters");
      return false;
    }
  ...
  if (GetNInternalQueues () != 1)
    {
      NS_LOG_ERROR ("PppPieQueueDisc needs 1 internal queue");
      return false;
    }

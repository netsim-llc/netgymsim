...
#include "ml-pf-ff-mac-scheduler.h"
#include <ns3/lte-vendor-specific-parameters.h>
#include <ns3/boolean.h>
#include <cfloat>
#include <set>

uint16_t g_ns_cellId_counter = 0; //store the cell ID of the enb

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("MlPfFfMacScheduler");
...
NS_OBJECT_ENSURE_REGISTERED (MlPfFfMacScheduler);



MlPfFfMacScheduler::MlPfFfMacScheduler ()
  : m_cschedSapUser (0),
    m_schedSapUser (0),
    m_timeWindow (99.0),
    m_nextRntiUl (0)
{
  ...
  m_cschedSapProvider = new MemberCschedSapProvider<MlPfFfMacScheduler> (this);
  m_schedSapProvider = new MemberSchedSapProvider<MlPfFfMacScheduler> (this);
  m_ffrSapProvider = 0;
  m_ffrSapUser = new MemberLteFfrSapUser<MlPfFfMacScheduler> (this);
  m_cellId = ++g_ns_cellId_counter;
...
TypeId
MlPfFfMacScheduler::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MlPfFfMacScheduler")
    .SetParent<FfMacScheduler> ()
    .SetGroupName("Lte")
    .AddConstructor<MlPfFfMacScheduler> ()
    .AddAttribute ("CqiTimerThreshold",
                   "The number of TTIs a CQI is valid (default 1000 - 1 sec.)",
                   UintegerValue (1000),
                   MakeUintegerAccessor (&MlPfFfMacScheduler::m_cqiTimersThreshold),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("HarqEnabled",
                   "Activate/Deactivate the HARQ [by default is active].",
                   BooleanValue (true),
                   MakeBooleanAccessor (&MlPfFfMacScheduler::m_harqOn),
                   MakeBooleanChecker ())
    .AddAttribute ("UlGrantMcs",
                   "The MCS of the UL grant, must be [0..15] (default 0)",
                   UintegerValue (0),
                   MakeUintegerAccessor (&MlPfFfMacScheduler::m_ulGrantMcs),
                   MakeUintegerChecker<uint8_t> ())
    .AddAttribute ("MeasurementInterval",
                   "LTE measurement interval",
                   TimeValue (Seconds(1.0)),
                   MakeTimeAccessor (&MlPfFfMacScheduler::m_measurementInterval),
                   MakeTimeChecker ())
    .AddAttribute ("MeasurementGuardInterval",
                   "Guard time interval between 2 measurements",
                   TimeValue (Seconds(0.0)),
                   MakeTimeAccessor (&MlPfFfMacScheduler::m_measurementGuardInterval),
                   MakeTimeChecker ())
    .AddTraceSource("RateMeasurement",
                "The transmission rate Measurement",
                MakeTraceSourceAccessor(&MlPfFfMacScheduler::m_rateMeasurement),
                "ns3::MlPfFfMacScheduler::RateMeasurementTracedCallback");
  ;
  return tid;
}
...
void
MlPfFfMacScheduler::SetFfMacCschedSapUser (FfMacCschedSapUser* s)
{
  m_cschedSapUser = s;
  Simulator::Schedule(m_measurementGuardInterval, &MlPfFfMacScheduler::MeasurementGuardEnd, this);

}

void
MlPfFfMacScheduler::SetFfMacSchedSapUser (FfMacSchedSapUser* s)
{
  m_schedSapUser = s;
}

FfMacCschedSapProvider*
MlPfFfMacScheduler::GetFfMacCschedSapProvider ()
{
  return m_cschedSapProvider;
}

FfMacSchedSapProvider*
MlPfFfMacScheduler::GetFfMacSchedSapProvider ()
{
  return m_schedSapProvider;
}

void
MlPfFfMacScheduler::SetLteFfrSapProvider (LteFfrSapProvider* s)
{
  m_ffrSapProvider = s;
}

LteFfrSapUser*
MlPfFfMacScheduler::GetLteFfrSapUser ()
{
  return m_ffrSapUser;
}

void
MlPfFfMacScheduler::DoCschedCellConfigReq (const struct FfMacCschedSapProvider::CschedCellConfigReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
MlPfFfMacScheduler::DoCschedUeConfigReq (const struct FfMacCschedSapProvider::CschedUeConfigReqParameters& params)
{
...
void
MlPfFfMacScheduler::DoCschedLcConfigReq (const struct FfMacCschedSapProvider::CschedLcConfigReqParameters& params)
{
  NS_LOG_FUNCTION (this << " New LC, rnti: "  << params.m_rnti);

  std::map <uint16_t, mlPfsFlowPerf_t>::iterator it;
      ...
      if (it == m_flowStatsDl.end ())
        {
          ...
          flowStatsDl.achievableRate = 0;
          m_flowStatsDl.insert (std::pair<uint16_t, mlPfsFlowPerf_t> (params.m_rnti, flowStatsDl));
          mlPfsFlowPerf_t flowStatsUl;
          ...
          flowStatsUl.achievableRate = 0;
          m_flowStatsUl.insert (std::pair<uint16_t, mlPfsFlowPerf_t> (params.m_rnti, flowStatsUl));
        }
    }
...
void
MlPfFfMacScheduler::DoCschedLcReleaseReq (const struct FfMacCschedSapProvider::CschedLcReleaseReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
MlPfFfMacScheduler::DoCschedUeReleaseReq (const struct FfMacCschedSapProvider::CschedUeReleaseReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
MlPfFfMacScheduler::DoSchedDlRlcBufferReq (const struct FfMacSchedSapProvider::SchedDlRlcBufferReqParameters& params)
{
...
void
MlPfFfMacScheduler::DoSchedDlPagingBufferReq (const struct FfMacSchedSapProvider::SchedDlPagingBufferReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
MlPfFfMacScheduler::DoSchedDlMacBufferReq (const struct FfMacSchedSapProvider::SchedDlMacBufferReqParameters& params)
{
  NS_LOG_FUNCTION (this);
  NS_FATAL_ERROR ("method not implemented");
  return;
...
int
MlPfFfMacScheduler::GetRbgSize (int dlbandwidth)
{
  for (int i = 0; i < 4; i++)
    {
...
unsigned int
MlPfFfMacScheduler::LcActivePerFlow (uint16_t rnti)
{
  std::map <LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
...
uint8_t
MlPfFfMacScheduler::HarqProcessAvailability (uint16_t rnti)
{
...
uint8_t
MlPfFfMacScheduler::UpdateHarqProcessId (uint16_t rnti)
{
  NS_LOG_FUNCTION (this << rnti);
...
void
MlPfFfMacScheduler::RefreshHarqProcesses ()
{
  NS_LOG_FUNCTION (this);
...
void
MlPfFfMacScheduler::DoSchedDlTriggerReq (const struct FfMacSchedSapProvider::SchedDlTriggerReqParameters& params)
{
  if (!m_connected)
  {
      Config::Connect("/NodeList/*/DeviceList/*/LteEnbRrc/ConnectionEstablished",
                  MakeCallback (&MlPfFfMacScheduler::NotifyConnectionEstablished, Ptr<MlPfFfMacScheduler>(this)));
    m_connected = true;
  }
  ...
  for (int i = 0; i < rbgNum; i++)
    {
      ...
      if (rbgMap.at (i) == false)
        {
          std::map <uint16_t, mlPfsFlowPerf_t>::iterator it;
          std::map <uint16_t, mlPfsFlowPerf_t>::iterator itMax = m_flowStatsDl.end ();
          double rcqiMax = 0.0;
          for (it = m_flowStatsDl.begin (); it != m_flowStatsDl.end (); it++)
            {
              if (i == 0 && m_measurementActive)
              {
                (*it).second.achievableRate = 0; //reset the throughput measurement from RBG 0
              }
                  ...
                  if (LcActivePerFlow ((*it).first) > 0)
                    {
                      ...
                      {
                        (*it).second.achievableRate += achievableRate; //adding the rate of this RBG to total rate
                      }
                    ...
                    {
                      // this UE has no data to transmit, only measure rate...
                      double achievableRate = 0.0;
                      uint8_t mcs = 0;
                      for (uint8_t k = 0; k < nLayer; k++)
                        {
  ...
  std::map <uint16_t, mlPfsFlowPerf_t>::iterator itStats;
...
void
MlPfFfMacScheduler::DoSchedDlRachInfoReq (const struct FfMacSchedSapProvider::SchedDlRachInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
MlPfFfMacScheduler::DoSchedDlCqiInfoReq (const struct FfMacSchedSapProvider::SchedDlCqiInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
double
MlPfFfMacScheduler::EstimateUlSinr (uint16_t rnti, uint16_t rb)
{
...
void
MlPfFfMacScheduler::DoSchedUlTriggerReq (const struct FfMacSchedSapProvider::SchedUlTriggerReqParameters& params)
{
      ...
      if (itStats != m_flowStatsUl.end ())
        {
          ...
          if(m_measurementActive)
          {
            (*itStats).second.achievableRate = (double)((m_amc->GetUlTbSizeFromMcs (uldci.m_mcs, m_cschedCellConfig.m_ulBandwidth) / 8) / 0.001);
          }
        }
      else
        {
...
void
MlPfFfMacScheduler::DoSchedUlNoiseInterferenceReq (const struct FfMacSchedSapProvider::SchedUlNoiseInterferenceReqParameters& params)
{
  NS_LOG_FUNCTION (this);
  return;
}
...
void
MlPfFfMacScheduler::DoSchedUlSrInfoReq (const struct FfMacSchedSapProvider::SchedUlSrInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
  return;
}
...
void
MlPfFfMacScheduler::DoSchedUlMacCtrlInfoReq (const struct FfMacSchedSapProvider::SchedUlMacCtrlInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
void
MlPfFfMacScheduler::DoSchedUlCqiInfoReq (const struct FfMacSchedSapProvider::SchedUlCqiInfoReqParameters& params)
{
  NS_LOG_FUNCTION (this);
...
// retrieve the allocation for this subframe
  ...
  switch (params.m_ulCqi.m_type)
    ...
    case UlCqi_s::PRACH:
      {
        NS_FATAL_ERROR ("MlPfFfMacScheduler supports only PUSCH and SRS UL-CQIs");
      }
      break;
    default:
...
void
MlPfFfMacScheduler::RefreshDlCqiMaps (void)
{
...
void
MlPfFfMacScheduler::RefreshUlCqiMaps (void)
{
  // refresh UL CQI  Map
...
void
MlPfFfMacScheduler::UpdateDlRlcBufferInfo (uint16_t rnti, uint8_t lcid, uint16_t size)
{
  std::map<LteFlowId_t, FfMacSchedSapProvider::SchedDlRlcBufferReqParameters>::iterator it;
...
void
MlPfFfMacScheduler::UpdateUlRlcBufferInfo (uint16_t rnti, uint16_t size)
{
...
void
MlPfFfMacScheduler::TransmissionModeConfigurationUpdate (uint16_t rnti, uint8_t txMode)
{
...
MlPfFfMacScheduler::MeasurementEvent ()
{
  ...
  for (uint32_t i = 0; i < m_flowStatsDl.size(); ++i)
  {
    ...
    for (it = m_flowStatsDl.begin (); it != m_flowStatsDl.end (); it++)
    {
      ...
      if(imsi == GetImsiMod((*it).first))
      {
        if((*it).second.achievableRate > 0 )//report new measurement
        {
          //ratePerRbg is the rate per RB group...
          //double ratePerRbg = std::round((*it).second.achievableRate/125e3);
          //double ratePerRbg = std::round((*it).second.lastAveragedThroughput/125);
          //int rbgSize = GetRbgSize (m_cschedCellConfig.m_dlBandwidth);
          //double rateOfAllRbs = ratePerRbg*m_cschedCellConfig.m_dlBandwidth/rbgSize;
          double rateOfAllRbs = std::round((*it).second.achievableRate/125e3);
          //std::cout << Now().GetSeconds () << " UE:"<< i << " rate per RBG:" << ratePerRbg  << " kbps " << " rbsize:" << m_cschedCellConfig.m_dlBandwidth << " rbgSize:" << rbgSize  << " rate of all RBs:" << rateOfAllRbs << std::endl;
          //Time nowTime = Now();
          m_rateMeasurement(rateOfAllRbs, imsi, true);
          (*it).second.achievableRate = 0; //reset to zero after this measurement.
        }
        else
        {
          //Time nowTime = Now();
          m_rateMeasurement(-1, imsi, true);
        }
      }
    }
  }

  for (uint32_t i = 0; i < m_flowStatsUl.size(); ++i)
  {
    uint64_t imsi = i+1;
    std::map <uint16_t, mlPfsFlowPerf_t>::iterator it;
  ...
  Simulator::Schedule(m_measurementGuardInterval, &MlPfFfMacScheduler::MeasurementGuardEnd, this);
  m_measurementActive = false;//enter measurement guard interval now.
}

void
MlPfFfMacScheduler::MeasurementGuardEnd ()
{
  Simulator::Schedule(m_measurementInterval, &MlPfFfMacScheduler::MeasurementEvent, this);
  m_measurementActive = true;//enter measurement now;
}

uint64_t
MlPfFfMacScheduler::GetImsiMod (uint16_t rnti)
{
  
  uint64_t imsi = 0;
  std::map <uint16_t, uint64_t>::iterator iter = m_rntiToImsiMap.find(rnti);
  if(iter != m_rntiToImsiMap.end())
  {
    imsi = iter->second;
  }
  // std::cout << "cell ID:"<< "1" << " rnti:" << rnti << " --> imsi:" << imsi << std::endl;
  return imsi;//0 stands for cannot find this user...
}

void
MlPfFfMacScheduler::NotifyConnectionEstablished (std::string context,
                                uint64_t imsi,
                                uint16_t cellid,
                                uint16_t rnti)
{

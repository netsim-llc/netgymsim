...
public:
  ...
  bool SetLinkDown(uint8_t cid);//link down due to one of the reasons.
  void SetLinkUp(uint8_t cid);//link up under all conditions.

  void AddLinkCid (uint8_t cid);
  std::vector<uint8_t> GetCidList();
  uint8_t GetDefaultLinkCid ();
  void SetBackupLinkCid (uint8_t cid);
  ...
  uint8_t m_qosTestDurationUnit100ms;//duration for qos testing
...
private:
  uint8_t m_defaultLinkCid;
  uint32_t m_backupLinkCid;
  ...
  uint32_t m_nodeId;
  bool m_fixDefaultLink;
  std::vector<uint8_t> m_cidList; //the list of cid for connected physic links
  bool m_preferLargeCidValue; //if true: large the cid -> high priority.
};

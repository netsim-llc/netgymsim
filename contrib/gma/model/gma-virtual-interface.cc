...
GmaVirtualInterface::GmaVirtualInterface (Time startTime)
{
	...
	//m_measurementManager = CreateObject<MeasurementManager> ();
...
GmaVirtualInterface::ReceiveDlSplitWeightAction (const json& action)
{
	...
	if (m_gmaRxControl->QosSteerEnabled())
	{
		...
		if(steerLinkCid != m_linkState->GetDefaultLinkCid())
		{
			m_measurementManager->GetObject<QosMeasurementManager>()->QuitQosMeasurement(m_linkState->GetBackupLinkCid()); //stop qos measurement for previous backup link
			m_linkState->SetBackupLinkCid(steerLinkCid);
			//qos testing enabled for one of the none default link.
			//chekck the prohibit timer...
			auto iter = m_linkState->m_cidToLastTestFailTime.find(m_linkState->GetBackupLinkCid());
		...
		else
		{
			...
			m_measurementManager->GetObject<QosMeasurementManager>()->QuitQosMeasurement(m_linkState->GetBackupLinkCid());
			m_qosTestingAllowed = false;
			Ptr<SplittingDecision> decision = m_gmaRxControl->GenerateTrafficSplittingDecision(m_linkState->GetDefaultLinkCid());//generate a tsu to switch traffic over default link
...
GmaVirtualInterface::SetFixedTxCid(uint8_t cid)
{
	...
	m_gmaTxControl->SetAlgorithm("DefaultLink");
}
...
GmaVirtualInterface::AddPhyLink(Ptr<Socket> socket, const Ipv4Address& phyAddr, uint8_t cid, int apId)
{
	...
	if(m_linkParamsMap.find(cid) == m_linkParamsMap.end())
	{
		...
		if (m_linkState->GetDefaultLinkCid() != cid)//not default link
		{
...
void
GmaVirtualInterface::ConfigureRxAlgorithm(uint8_t splittingBurst)
{
...
GmaVirtualInterface::Transmit (Ptr<Packet> packet)
{
	...
	if(m_clientRoleEnabled && m_gmaRxControl->QosSteerEnabled() )//client side && QOS enabled
	{
		...
		while (iter!= m_linkParamsMap.end())
		{
			...
			if (cid != m_linkState->GetDefaultLinkCid())//not primary link
			{					
	...
	if(m_duplicateMode)
	{
		...
		while(iter!=m_linkParamsMap.end())
		{
			...
			if(m_linkState->IsLinkUp(cid))
			{
				...
				{
					SendByCid(cid, dummyP, TOS_AC_VI); //udp traffic use video.
				}
				else
				{
					SendByCid(cid, dummyP, TOS_AC_BE); //tcp and other traffic use Best effort.
				}
	...
	else
	{
		...
		uint8_t cid = m_linkState->GetDefaultLinkCid();
		if (m_gmaTxControl->GetSplittingBurstFromTsu() == 1)
		{
			//steer mode:
		...
		if(markIter != m_linkParamsMap.end() && m_gmaRxControl->QosFlowPrioritizationEnabled())// find action for this user
		{
			...
			{
				SendByCid(cid, dummyP, TOS_AC_VI);//qos, mark as video
			}
			else
			{
				SendByCid(cid, dummyP, TOS_AC_BE);//nqos mark as best effort
			}
		}
		else
		{
			if (ipv4Header.GetProtocol() == 17)
			{
				SendByCid(cid, dummyP, TOS_AC_VI); //udp traffic use vidoe.
			}
			else
			{
				SendByCid(cid, dummyP, TOS_AC_BE); //tcp and other traffic use Best effort.
		...
		for(uint32_t ind = 0; ind < dupCidList.size(); ind++)
		{
			...
			if(m_linkState->IsLinkUp(cid))
			{
				...
				//m_linkParamsMap[cid]->m_socket->SendTo (dummyP, 0 ,InetSocketAddress (m_linkParamsMap[cid]->m_ipAddr, START_PORT_NUM+cid));
				SendByCid(cid, dummyP, TOS_AC_BK);//testing packet, mark as background
...
GmaVirtualInterface::Receive (Ptr<Packet> packet, const Ipv4Address& phyAddr, uint16_t fromPort)
{
	...
	else
	{
		...
		if(gmaHeader.GetFlowId() == QOS_FLOW_ID)//QOS mode
		{
			m_receiveDuplicateFlow = true;//the sender is sending duplicate packets
			if(m_useLsnReordering == true)
			{
		...
		if(gmaHeader.GetFlowId() == QOS_FLOW_ID && m_discardBackupLinkPackets && cid != m_linkState->GetDefaultLinkCid())
		{
...
GmaVirtualInterface::QosViolationMsg (uint8_t direction)
{
	...
	MxControlHeader mxHeader;
	mxHeader.SetType (10);
	mxHeader.SetConnectionId (m_linkState->GetDefaultLinkCid());
	mxHeader.SetSequenceNumber(m_mxCtrSeqNumber++);
	mxHeader.SetTimeStamp(now & 0xFFFFFFFF);
	mxHeader.SetFlowId(QOS_FLOW_ID);
	mxHeader.SetTrafficDirection(direction);
...
GmaVirtualInterface::SendCtrlMsg (const MxControlHeader& header, bool retx)
{
	...
	if(header.GetType() == 5)
	{
		...
		while (iter!= m_linkParamsMap.end())//iterate all links
		{
			if(splitVector.at(index) !=0 )//traffic over this link
			{
				...
				if(m_gmaRxControl->QosSteerEnabled() )//QOS enabled
				{
	...
	if (retx == false)//this is the first transmission of this control message
	{
		...
		m_ctrRto = INITIAL_CONTROL_RTO; //reset rto timer;
		if(m_measurementManager->GetMaxRttMs() !=0 )// == 0 means one or more links dont have measurement.
		{
...
GmaVirtualInterface::RetxCtrlMsgExpires(uint16_t csn)
{
	...
	{
		//std::cout << Now().GetSeconds() << " " << this << " first sn:" <<  csn << " sn:" << m_mxCtrSeqNumber<< " expires no ack ================== rto: " <<m_ctrRto << " msg:" << m_txedCtrQueue[csn]->m_mxControlHeader <<"\n";
...
GmaVirtualInterface::RecvCtrlMsg (Ptr<Packet> packet, const Ipv4Address& phyAddr)
{
	...
	if (mxHeader.GetType () == 1)//this is a Probe msg, we need to send back ACK
	{
		...
		{
			SendByCid(cid, ackPacket);//normal Probe
		}
		else
		{
			...
			if (CsnDiff(mxHeader.GetSequenceNumber(), m_maxTsuSeqNum) > 0)
			{
				if(m_gmaRxControl->QosSteerEnabled())
				{
					...
					if(m_clientRoleEnabled)
					{
						...
						if(currentCid != m_linkState->GetDefaultLinkCid())
						{
				...
				{
					//split mode & default link cid changed, we need to send back a end marker...
					if(m_clientRoleEnabled && m_gmaRxControl->QosSteerEnabled() && m_gmaTxControl->GetDeliveryLinkCid() != m_linkState->GetDefaultLinkCid())
					{
	...
	else if (mxHeader.GetType () == 6 || mxHeader.GetType () == 7) // i use all ACK asd TSA to updat rtt and owd.
	{
		...
		if (ackFound)
		{
			...
			else if(sendMsgType == 9)//ack for qos testing start notify
			{
				...
				if (sendMsg.GetConnectionId() != m_linkState->GetDefaultLinkCid())//test over backup link
				{
					if(sendMsg.GetTrafficDirection() == 0 && m_serverRoleEnabled)//downlink testing
					{
						//In downlink qos, server does nothing after rcv request ACK
					}
					else if(sendMsg.GetTrafficDirection() ==1 && m_serverRoleEnabled)//uplink testing
					{
						...
						bool success = m_measurementManager->GetObject<QosMeasurementManager>()->QosMeasurementStart(sendMsg.GetConnectionId(), sendMsg.GetTestDuration());
			...
			m_measurementManager->UpdateOwdFromAck(Now().GetMilliSeconds() - mxHeader.GetTimeStamp(), mxHeader.GetConnectionId());// update OWD
			...
			if (mxHeader.GetType () == 7 && m_rxMode && CsnDiff(mxHeader.GetSequenceNumber(), m_maxTsaSn) > 0)//TSA msg
			{
				...
				if(m_gmaRxControl->QosSteerEnabled() )//client or server side && QOS enabled
				{
					...
					if (mxHeader.GetConnectionId() != m_linkState->GetDefaultLinkCid())
					{
	...
	else if (mxHeader.GetType () == 8)//this is a QoS Testing Request, we need to send back ACK
	{
		...
		if(mxHeader.GetTrafficDirection() == 0 && m_serverRoleEnabled)
		{
		...
		else if (mxHeader.GetTrafficDirection() == 1 && m_serverRoleEnabled)
		{
			//uplink, server does nothing
		}
		else
		{
			NS_FATAL_ERROR("Did not implement this case");
		}
	...
	else if (mxHeader.GetType () == 9)//this is a QoS Testing Start notify, we need to send back ACK
	{
		...
		uint8_t qosDuration = m_linkState->m_qosTestDurationUnit100ms;
		Simulator::Schedule(MilliSeconds(qosDuration*100), &GmaVirtualInterface::QosTestingSessionEnd, this);//stop qos testing after qosDurationS
	...
	else if (mxHeader.GetType () == 10)//this is a QoS Violation MSG
	{
		...
		if(failCid != m_linkState->GetDefaultLinkCid())
		{
...
void
GmaVirtualInterface::SetQosRequirement (uint64_t qos_delay, uint64_t tresh1, uint64_t thresh2, double delayTarget, double lossTarget)
{

...
GmaTxControl::SetAlgorithm (std::string algorithm)
{
	...
	{
		m_algorithm = GmaTxControl::DefaultLink;
	} 
	else
	{
...
{	
	...
	else if (m_algorithm == GmaTxControl::DefaultLink)
	{
		cid = m_linkState->GetDefaultLinkCid();
	}
	else
	{
...
{
	uint8_t cidValue = m_linkState->GetDefaultLinkCid();
...
{
	std::cout << this << " start Qos Testing for " << +testDuration << " *100ms" << std::endl;
	if(m_cidDupVector.empty())
	{
		//we will send duplicated packet over this  cid for testing.
		m_cidDupVector.push_back(cid);
		Simulator::Schedule(MilliSeconds(testDuration*100), &GmaTxControl::StopQosTesting, this, cid);
	}
	else
	{
		//we only support testing over 1 link in a session
		//testing is running. ..this happens when qos request is retransmitted...
		NS_FATAL_ERROR("testing running. This may due to CTRL msg retransmission timeout");
	}
...
{
	//stop all testing...
	m_cidDupVector.erase(std::remove(m_cidDupVector.begin(), m_cidDupVector.end(), cid), m_cidDupVector.end()); 
}
...
GmaTxControl::QoSTestingRequestCheck (uint8_t cid)
{
	...
	if(m_cidVector.size() == 0)
	{
		//no tsu received yet, all traffics goes to primary link
		trafficOverBackupLinks = 0;
		trafficOverPrimaryLink = 1;
	}
	else
	{
		//we already received tsu, check if the traffic over primary or backup links.
		uint8_t primaryCid = m_linkState->GetDefaultLinkCid();

		for (uint8_t ind = 0; ind < m_cidVector.size(); ind++)
		{
			if(primaryCid == m_cidVector.at(ind))
			{
				trafficOverPrimaryLink += 1;
			}
			else
			{
				trafficOverBackupLinks += 1;
			}
		}
	...
	if(trafficOverBackupLinks == 0) // traffic over the primary link
	{
		if(m_linkState->m_cidToLastTestFailTime.empty())
		{
			//we can start a new qos test
			m_linkState->m_cidToLastTestFailTime[cid] = Now();

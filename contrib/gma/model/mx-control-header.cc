...
MxControlHeader::Print (std::ostream &os) const
{
  ...
  else if (m_type == 8 || m_type == 9)//QOS messages
  {
  os<<"Type=["<<(int)m_type <<"] CID=["<<(int)m_connectionId<< "] Sequence #=[" << (int)m_sequenceNumber
   <<"] TimeStamp=["<<(int)m_timeStamp<<"] FlowId=[" << (int)m_flowId  <<"] Traffic Direction=["<<(int)m_trafficDirection<<"] Test Duration=["
   <<(int)m_testDuration<<"]";
    if(m_type == 8)
    {
      os<<"ChannelId=["<<(int)m_channelId <<"]";
    }
  }
  else if (m_type == 10)//QOS violation
  {
  os<<"Type=["<<(int)m_type <<"] CID=["<<(int)m_connectionId<< "] Sequence #=[" << (int)m_sequenceNumber
   <<"] TimeStamp=["<<(int)m_timeStamp<<"] FlowId=[" << (int)m_flowId  <<"] Traffic Direction=["<<(int)m_trafficDirection<<"]";
  }
  else
  {
...
MxControlHeader::Serialize (Buffer::Iterator start) const
{
  ...
  else if (m_type == 8 || m_type == 9) // QOS MSG
  {
      start.WriteU16 (m_sequenceNumber);
      start.WriteU32 (m_timeStamp);
      start.WriteU8 (m_flowId);
      start.WriteU8 (m_trafficDirection);
      start.WriteU8 (m_testDuration);
      if (m_type == 8)
      {
...
MxControlHeader::Deserialize (Buffer::Iterator start)
{
  ...
  else if (m_type == 8 || m_type == 9) //QOS MSG
  {
    ...
    m_timeStamp = start.ReadU32 ();
    m_flowId = start.ReadU8 ();
    m_trafficDirection= start.ReadU8 ();
    m_testDuration = start.ReadU8 ();
    if(m_type == 8)
    {
      m_channelId = start.ReadU8 ();
    }
  }
  else if (m_type == 10) //QOS Violation
  {

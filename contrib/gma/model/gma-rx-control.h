...
public:
  ...
  */
  ...
  enum GmaRxAlgorithm
  {
    ...
    DefaultLink = 2,      // select the default link
    ...
    gma2 = 6,  // split case balance based on the delay violation, steer case select the link with min owd
  };
  const double PKT_NUM_WEIGHT = 0.8; //the estimated pkt n = last interval pkt * PKT_NUM_WEIGHT. In the future, we need prediction.
  double m_qosDelayViolationTarget;
  double m_qosLossTarget;
...
private:
  ...
  bool m_enableAdaptiveStep;
  const int STEP_THRESH  = 4; // increase step if keep decreasing more than STEP_THRESH time
  bool m_enableStableAlgorithm;
  bool m_enableLossAlgorithm;
  Ptr<LinkState> m_linkState;

  bool m_enableQosFlowPrioritization;

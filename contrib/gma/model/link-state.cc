...
LinkState::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LinkState")
    .SetParent<Object> ()
    .SetGroupName("Gma")
    .AddAttribute ("DefaultLinkCid",
               "The cid of the default ink",
               UintegerValue (WIFI_CID),
               MakeUintegerAccessor (&LinkState::m_defaultLinkCid),
               MakeUintegerChecker<uint8_t> ())
	.AddAttribute ("BackupLinkCid",
               "The cid of the backup ink",
               UintegerValue (WIFI_CID),
               MakeUintegerAccessor (&LinkState::m_backupLinkCid),
               MakeUintegerChecker<uint8_t> ())
...
bool
LinkState::SetLinkDown(uint8_t cid)
{
	if(m_fixDefaultLink)
	{
	...
	{
		if(cid == m_defaultLinkCid)//this link is failed we need to find another one for default tx
		{

			if(m_preferLargeCidValue)//high priority -> large cid
			{
				//find the max cid of ok links
				uint8_t maxOkCid = UINT8_MAX;
				for (uint8_t ind = 0; ind < m_cidList.size(); ind++)//find a link with min cid still ok as default
				{
					if(IsLinkUp(m_cidList.at(ind)) && m_cidList.at(ind) > maxOkCid)
					{
						maxOkCid = m_cidList.at(ind);
					}
				}

				if(maxOkCid != UINT8_MAX)
				{
					m_defaultLinkCid = maxOkCid;
				}
				
			}
			else//high priority -> small cid
			{
				//find min cid of ok links
				...
				{
					m_defaultLinkCid = minOkCid;
				}
			}
			//else all links failed, do nothing...
			std::cout << "LINK STATE: change default link from " << +cid << " to :" << +m_defaultLinkCid << "\n";
		}
		else
		{
			std::cout << "LINK STATE: default link no change: " << +m_defaultLinkCid << "\n";
		}
		return true;
	}
...
void
LinkState::SetLinkUp(uint8_t cid)
{
	if(m_fixDefaultLink)
	{
		//do not update the default link ID
	...
	else
	{
		...
		std::cout << "LINK STATE: change default link to :" << +m_defaultLinkCid << "\n";
	}
...
{
	return m_defaultLinkCid;
}

void
LinkState::SetBackupLinkCid(uint8_t cid)
{
	m_backupLinkCid = cid;
}

uint8_t
LinkState::GetBackupLinkCid()
{
	return m_backupLinkCid;
}

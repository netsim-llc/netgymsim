...
public:
  ...
  uint32_t m_rtt = 0; //ms
  uint32_t m_owd_same_time_as_rtt = 0; //owd measured the same time as m_rtt.
  ...
  uint32_t m_ackOwd = 0;
  uint32_t m_numOfDelayViolationDataPacketsPerInterval; //the number of data packets violate queueing delay, e.g., owd - min_owd.
  ...
  void UpdateLastAckOwd(uint32_t owdMs);
...
private:
  ...
  Time m_lastMinRttUpdateTime = Seconds(0.0);
